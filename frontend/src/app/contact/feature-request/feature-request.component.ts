...
/**
 * Component responsible for rendering and handling the feature request form.
 * Allows users to submit feature requests, including descriptions, email addresses, and file attachments.
...
export class FeatureRequestComponent {
  @Input() values!: FormValuesFeature;
  ...
  maxCharactersFeatureDescription = 1800;
  maxCharactersMailAddress = 255;
  checkIfSubmitted = false;

  /**
   * Constructs the `FeatureRequestComponent`.
  ...
  /**
   * Validates the form and submits the feature request data if valid. Displays appropriate alerts for validation errors.
